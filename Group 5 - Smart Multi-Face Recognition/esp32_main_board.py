...
try:
    ...
    try:
        import st7735
        tft = st7735.TFT(spi, PIN_TFT_DC, PIN_TFT_RST, PIN_TFT_CS)
        try:
            tft.initg()
        except:
            tft.initr()  # Fallback
        ...
        tft.rgb(True)
        try:
            # Set to landscape (rotation 3 = right-side up for most models)
            tft.rotation(3) 

            # Fix for specific clone displays where the internal hardware matrix 
            # is horizontally mirrored natively. 0x20 corrects the chirality without flipping vertically.
            tft._writecommand(0x36)
            tft._writedata(bytearray([0x20]))
        except:
            pass
            
        tft.fill(st7735.TFTColor(0, 0, 0))
        tft_available = True
        print("[OK] TFT display (ST7735) initialized")
    except ImportError:
        print("[WARNING] No TFT driver found (st7735.py)")
...
# Color constants (ST7735 format)
try:
    import st7735
    COLOR_BLACK   = st7735.TFTColor(0, 0, 0)
    COLOR_WHITE   = st7735.TFTColor(255, 255, 255)
    COLOR_GREEN   = st7735.TFTColor(0, 255, 0)
    COLOR_RED     = st7735.TFTColor(255, 0, 0)
    COLOR_BLUE    = st7735.TFTColor(0, 0, 255)
    COLOR_YELLOW  = st7735.TFTColor(255, 255, 0)
    COLOR_CYAN    = st7735.TFTColor(0, 255, 255)
    COLOR_PURPLE  = st7735.TFTColor(255, 0, 255)
    COLOR_ORANGE  = st7735.TFTColor(255, 165, 0)
except ImportError:
    # Colors if driver is missing
    COLOR_BLACK = 0; COLOR_WHITE = 0; COLOR_GREEN = 0; COLOR_RED = 0
    COLOR_BLUE = 0; COLOR_YELLOW = 0; COLOR_CYAN = 0; COLOR_PURPLE = 0; COLOR_ORANGE = 0

# Very basic font for ST7735 to prevent ImportError
sysfont = {
    'Start': 32, 'End': 126, 'Width': 6, 'Height': 8,
    'Data': bytearray([
        
    ])
}
...
def tft_text(text, x, y, color=COLOR_WHITE, scale=1):
    """Draw text on display using the bundled font."""
    if tft_available:
        try:
            # We use the built-in sysfont we defined above
            tft.text((x, y), text, color, sysfont, scale)
        except Exception as e:
            pass
