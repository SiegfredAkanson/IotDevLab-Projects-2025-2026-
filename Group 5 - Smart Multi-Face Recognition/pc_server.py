...
ESP32_CAM_IP = "10.233.218.13"  # Your ESP32-CAM's IP
...
CONFIDENCE_THRESHOLD = 125 # Tuned for high accuracy
...
def capture_frame():
    ...
    if using_esp32_cam:
        try:
            resp = requests.get(ESP32_CAM_URL, timeout=2.0)
...
def set_camera_setting(var, val):
    """Send control command to ESP32-CAM (e.g., framesize, led_intensity, vflip)."""
    if using_esp32_cam:
        try:
            url = f"http://{ESP32_CAM_IP}/control?var={var}&val={val}"
            requests.get(url, timeout=2)
        except Exception as e:
            print(f"[CAMERA SETTING ERROR] Failed to set {var}={val}: {e}")
...
def recognize_faces():
    ...
    # Turn flashlight ON during scan
    set_camera_setting("led_intensity", 255)
    ...
    # Turn flashlight OFF after scan
    set_camera_setting("led_intensity", 0)
...
def enroll_face(person_name, num_samples=50):
    """
    ...
    # Turn flashlight ON during enrollment
    set_camera_setting("led_intensity", 255)
    ...
    print(f"  Capturing {num_samples} face samples for high accuracy...")
    print("  !!! PLEASE SLOWLY TURN YOUR HEAD LEFT, RIGHT, UP, AND DOWN !!!")
    print("  !!! ENSURE YOU HAVE GOOD LIGHTING ON YOUR FACE !!!")
    ...
    if captured >= num_samples:
        ...
        set_camera_setting("led_intensity", 0) # Turn off flashlight
        retrain_model()
